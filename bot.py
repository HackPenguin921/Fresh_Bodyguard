...
async def story(ctx, who: str = None):
    ...
    places = [
        ...
        "ピリジャーの前線基地", "溶岩の海", "雪山", "監獄", "チェスト部屋", "図書館", "要塞",
        "砂漠の寺院", "キノコ島", "スポナー部屋", "儀式の間", "巨大スライムの巣", "古代都市", "遠い海底",
        "隠し部屋", "天空の神殿", "村の地下室", "バイオームの境界", "スニッファーの巣", "村の祭壇", "迷子のネザーゲート"
    ]

    actions = [
        "クリーパーに話しかけた", "TNTを設置した", "村人を叩いた", "ゾンビピッグマンを挑発した",
        "ダイヤを拾った", "ネコを手懐けた", "ベッドを壊した", "ポーションを全部飲んだ",
        "コマンドブロックを触った", "レッドストーン回路を爆発させた", "Witherを召喚した",
        "ドラゴンに投げキッスした", "トロッコで暴走した", "彼女を作った", "エリトラで空を飛んだ",
        "ボートでネザーを渡った", "ラマを怒らせた", "エンダーパールを投げまくった",
        "旗を燃やした", "スケルトンと協定を結んだ", "ブロックを無限に積んだ", "バグ利用を試みた",
        "ビーコンを盗んだ", "バケツで溶岩を飲んだ", "地図を全部捨てた", "カボチャをかぶったまま生活した",
        "空腹状態で踊った", "村を監視した", "スニッファーを驚かせた", "ビーコンに爆竹を挿した"
    ]

    results = [
        "家が吹き飛んだ", "全ロスした", "敵が大群で襲ってきた", "エンダードラゴンが現れた",
        "サーバーが落ちた", "村人に通報された", "ネザーゲートが消えた", "リアルの世界にバグった",
        "MODが暴走した", "BANされた", "天空から落下した", "クリーパーにプロポーズされた",
        "コマンドが暴走した", "チートがバレた", "ベッドが爆発した", "無限ループに入った",
        "ネコが怒った", "ラマに唾を吐かれた", "天空にテレポートした", "バグ技が成功してしまった",
        "周囲が全部ジャガイモになった", "透明になった", "スティーブと融合した", "空間が崩壊した",
        "村人が反乱を起こした", "どこかへ消えた", "なぜかヒカキンになった", "世界の果てに到達した",
        "地面が消えた", "全てのアイテムがニワトリに変わった"
    ]
